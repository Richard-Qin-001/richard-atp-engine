...
struct Term {
    SymbolId symbol_id_;        ///< Index into the SymbolTable
    std::vector<TermId> args_;  ///< Argument sub-term IDs

    /// Number of arguments.
    [[nodiscard]] uint16_t arity() const { return static_cast<uint16_t>(args_.size()); }
};
